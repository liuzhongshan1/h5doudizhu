function adduser(data){
    ...
    var html='<div class="member member'+indexuser+'" id="user'+data.user.id+'">'
      ...
      if(isguanzhan==0)
     {
      ...
      html+='<img src="http://cdn6.8848dai.com/images/chajian/y1.png" style="width: 100%;" />'
      html+='<div class="innerText" style="position: absolute; top: 1px; left: 11.5px; width: 100%; height: 100%;line-height: 9vw;font-size: 4.5vw;text-align: center;color:#fff;font-weight: bold;">准备</div>'
      html+="</div>"   
        }
      ...
      ctx.font = "30px bolder 微软雅黑";
        ...
        for(var i=0;i<sj.user.length;i++){
            if(i>1){
                ...
                ctx.fillRect(142,503-(0-(i-2)*70),515,67);
            }
            var user=sj['user'][i];
            ctx.font = "31px 微软雅黑";
            ctx.fillStyle = "#d8cb66";
            ctx.fillText(decode64(user.nickname).substring(0, 10), 225, 415+i*67);
            if(user.dqjf>0){
                user.dqjf='+'+user.dqjf;
            }else{
                ctx.fillStyle = "#fff";
                ctx.fillText(decode64(user.nickname).substring(0, 10), 225, 415+i*67);
                ctx.fillText(user.dqjf, 555, 415-(0-i*67));
            }
            ctx.fillText(user.dqjf, 555, 415-(0-i*67));
        }
