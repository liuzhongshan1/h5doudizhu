      ...
      ctx.font = "27px bolder 微软雅黑";
        ...
        for(var i=0;i<sj.user.length;i++){
            if(i>1){
                ...
                ctx.fillRect(142,519-(0-(i-2)*75),516,70);
            }
            var user=sj['user'][i];
            ctx.font = "31px bolder 微软雅黑";
            ctx.fillStyle = "#d8cb66";
            ctx.fillText(decode64(user.nickname).substring(0, 10), 225, 420+(i*74));
            if(user.dqjf>0){
                user.dqjf='+'+user.dqjf;
            }else{
                ctx.fillStyle = "#fff";
                ctx.fillText(decode64(user.nickname).substring(0, 10), 225, 420+(i*74));
                ctx.fillText(user.dqjf, 531, 420+(i*74));
            }
            ctx.fillText(user.dqjf, 531, 420+(i*74));
        }
