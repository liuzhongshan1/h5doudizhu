  ...
  //var credits = (data.mode == 1 ? parseInt(data.user.credits) : 0) + parseInt(data.user.dqjf);
    if(indexuser >4){
        var html='<div class="memberUp memberUp'+indexuser+'"  id="user'+data.user.id+'">';
            ...
            //html+='<div><div><img src="'+data.user.img+'" class="avatar" /> <div class="info"><div class="back"></div> <div class="name">'+data.user.nickname+'</div> <div class="score" data-val="0" data-dqjf="' + data.user.dqjf + '" data-credits="' + data.user.credits + '" data-mode="' + data.mode + '">'+credits+'</div></div></div>';
        ...
        //$('#member .member'+indexuser+' .score').html(credits).data("dqjf", data.user.dqjf).data("credits", data.user.credits).data("mode", data.mode);
...
function gxjifen(data){
    ...
    if(indexuser<=0){
  	...
  	var $bscore = $('#member .member' + indexuser + ' .score');
  	//var mode = parseInt($bscore.data("mode"));
	//$bscore.html((mode == 1 ? credits : 0) + data.dqjf).data("dqjf", data.dqjf);
  	$bscore.html(data.dqjf);
}
function qbank(zt){
    if(zt == 1){
        operationButton('9');
...
function jibiover2(){
    ...
    $('#member .member').each(function(){
        if(!$(this).find('.avatar').is(':hidden')){
            ...
            //var lsjf = parseInt($score.data("dqjf"));
            var dqjf = parseInt($score.attr('data-val'));
          	//var mode = parseInt($score.data("mode"));
			//$score.html((mode == 1 ? credits : 0) + dqjf).data("dqjf", dqjf);
          	var lsjf = parseInt($score.html());
          	$score.html(dqjf);
        if(dqjf - lsjf > 0){
      ...
      ctx.font = "28px 微软雅黑";
        ...
        for(var i=0;i<sj.user.length;i++){
            if(i>1){
                ...
                ctx.fillRect(120,553-(0-(i-2)*78),561,70);
            }
            var user=sj.user[i];
            console.log(user)
            ctx.font = "31px bolder 微软雅黑";
            ctx.fillStyle = "#d8cb66";
            ctx.fillText(user.nickname.substring(0,8), 225, 436+(i*80));
            if(user.dqjf>0){
                user.dqjf='+'+user.dqjf;
            }else{
                ctx.fillStyle = "#fff";
                ctx.fillText(user.nickname.substring(0,8), 225, 436+(i*80));
                ctx.fillText(user.dqjf,  531, 436+(i*80));
            }
            ctx.fillText(user.dqjf,  531, 436+(i*80));
        }
