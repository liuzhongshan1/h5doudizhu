...
function adduser(data){
    ...
    html+='<img id="banker213612" src="/app/img/bull9/bull_banker_bg.png" class="background jiurenniuniu-background" /> '
     ...
     html+='<img id="bankerAnimate1'+indexuser+'" src="/app/img/bull9/bull_banker_animate.png" class="jiurenniuniu-background2" /><div class="isReady" style=""><img src="http://goss.fexteam.com/files/d_25/images/game/ready.png " class="ready" style="display: none;"></div></div>';
     $('#member').append(html);
     if(data.user.zt==1){
...
function overroom(data){
    ...
    img.onload = function(){
        ...
        ctx.fillText("房间号:"+sj.id, 190,285);
        ctx.fillText(time1, 370, 285);
        ctx.fillText(sj.zjs+"局", 570, 285);
        ...
        for(var i=0;i<sj.user.length;i++){
            if(i>1){
                ...
                ctx.fillRect(140,519-(0-(i-2)*75),521,70);
            }
            var user=sj['user'][i];
            ctx.font = "32px 微软雅黑";
            ctx.fillStyle = "#d8cb66";
            //ctx.fillText(user.id, 195, 380+(i*75));
            ctx.fillText(decode64(user.nickname).substring(0,10), 219, 420+(i*75));
            if(user.dqjf>0){
                user.dqjf='+'+user.dqjf;
            }else{
                ctx.fillStyle = "#fff";
                ctx.fillText(decode64(user.nickname).substring(0,10), 219, 420+(i*75));
                ctx.fillText(user.dqjf, 571, 420+(i*75));
            }
            ctx.fillText( user.dqjf, 571, 420+(i*75));
        }
